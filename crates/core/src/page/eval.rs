...
impl Page {
	...
	pub async fn evaluate(&self, expression: &str) -> Result<()> {
		self.main_frame().await?.frame_evaluate_expression(expression).await
	}
	...
	pub async fn evaluate_value(&self, expression: &str) -> Result<String> {
		self.main_frame().await?.frame_evaluate_expression_value(expression).await
	}
	...
	pub async fn evaluate_json(&self, expression: &str) -> Result<serde_json::Value> {
		self.main_frame().await?.frame_evaluate_expression_json(expression).await
	}
