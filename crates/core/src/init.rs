...
pub async fn initialize_playwright(connection: &Arc<Connection>) -> Result<Arc<dyn ChannelOwner>> {
	...
	connection.register_object(Arc::from(""), root.clone()).await;

	tracing::debug!("Root object registered, sending initialize message");

	let root_typed = root.downcast_ref::<Root>().expect("Root object should be Root type");

	// Send initialize with timeout
	let response = tokio::time::timeout(Duration::from_secs(30), root_typed.initialize())
		.await
		.map_err(|_| Error::Timeout("Playwright initialization timeout after 30 seconds".to_string()))??;

	// Extract Playwright GUID from response
	let playwright_guid = response["playwright"]["guid"]
		.as_str()
		.ok_or_else(|| Error::ProtocolError("Initialize response missing 'playwright.guid' field".to_string()))?;
	...
	playwright_obj
		...
		.ok_or_else(|| Error::ProtocolError(format!("Object with GUID '{}' is not a Playwright instance", playwright_guid)))?;
...
impl ObjectFactory for DefaultObjectFactory {
	fn create_object(&self, parent: ParentOrConnection, type_name: String, guid: Arc<str>, initializer: Value) -> AsyncChannelOwnerResult<'_> {
		Box::pin(async move { crate::object_factory::create_object(parent, type_name, guid, initializer).await })
	}
}
