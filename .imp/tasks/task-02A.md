...
1. Positional argument order (URL, selector) creates UX friction when using CDP
1. Click command reports wrong navigation status
1. `--no-context` mode can't operate on current page when connected via CDP
...
1. Debug and fix by running `cargo build`, `cargo check`, reading errors, iterating until it compiles
1. Test changes with `cargo test` and manual CLI testing
1. Complete the full implementation; do not stop at partial solutions
1. Create integration tests that verify each fix works
...
1. **Over-eager selector detection:** Don't treat all non-URL strings as selectors. Be conservative - only apply heuristics when the pattern is clearly CSS-like.

1. **Ignoring test failures:** Every fix must have a passing test before moving on.

1. **Coupling too tightly:** The args.rs detection helper should be independent of context state. Don't mix concerns.

1. **Silent behavior changes:** If behavior changes (e.g., selector detection), log at debug level so users can understand what happened.
